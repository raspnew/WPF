...
class Interactor(nn.Module):
    def __init__(self, in_planes, splitting=True,
        ...
        if self.kernel_size % 2 == 0:
            ...
            pad_l_3 = self.dilation * (self.kernel_size - 2 - 2) // 2 + 1
            pad_r_3 = self.dilation * (self.kernel_size-2) // 2 + 1
            pad_l_1 = self.dilation * (self.kernel_size - 2 - 2 - 2) // 2 + 1
            pad_r_1 = self.dilation * (self.kernel_size - 2 - 2) // 2 + 1
        else:
            pad_l = self.dilation * (self.kernel_size - 1) // 2 + 1 # we fix the kernel size of the second layer as 3.
            pad_r = self.dilation * (self.kernel_size - 1) // 2 + 1
            pad_l_3 = self.dilation * (self.kernel_size - 1 - 2) // 2 + 1
            pad_r_3 = self.dilation * (self.kernel_size - 1 - 2) // 2 + 1
            pad_l_1 = self.dilation * (self.kernel_size - 1 - 2 - 2) // 2 + 1
            pad_r_1 = self.dilation * (self.kernel_size - 1 - 2 - 2) // 2 + 1
        ...
        ######################################3*3######################################
        modules_phi_3 += [
            nn.ReplicationPad1d((pad_l_3, pad_r_3)),
            nn.Conv1d(in_planes * prev_size, int(in_planes * size_hidden),
                      kernel_size=self.kernel_size - 2, dilation=self.dilation, stride=1, groups=self.groups),
            nn.LeakyReLU(negative_slope=0.01, inplace=True),
            nn.Dropout(self.dropout),
            nn.Conv1d(int(in_planes * size_hidden), in_planes,
                      kernel_size=3, stride=1, groups=self.groups),
        ...
        modules_pool += [
            nn.ReplicationPad1d((pad_l_3, pad_r_3)),
            nn.Conv1d(in_planes * prev_size * 3, int(in_planes * size_hidden),
                      kernel_size=self.kernel_size - 2, dilation=self.dilation, stride=1, groups=self.groups),
            nn.LeakyReLU(negative_slope=0.01, inplace=True),
            nn.Dropout(self.dropout),
            nn.Conv1d(int(in_planes * size_hidden), in_planes,
        ...
        ############################1*1#######################################################
        modules_phi_1 += [
        nn.ReplicationPad1d((pad_l_1, pad_r_1)),
        nn.Conv1d(in_planes * prev_size, int(in_planes * size_hidden),
                    kernel_size=self.kernel_size-4, dilation=self.dilation, stride=1, groups= self.groups),
        nn.LeakyReLU(negative_slope=0.01, inplace=True),
        nn.Dropout(self.dropout),
        nn.Conv1d(int(in_planes * size_hidden), in_planes,
                    kernel_size=3, stride=1, groups= self.groups),
        ...
        self.phi = nn.Sequential(*modules_phi)
        self.psi = nn.Sequential(*modules_psi)
        self.phi_3 = nn.Sequential(*modules_phi_3)
        self.psi_3 = nn.Sequential(*modules_psi_3)
        self.phi_1 = nn.Sequential(*modules_phi_1)
        self.psi_1 = nn.Sequential(*modules_psi_1)
        self.pool = nn.Sequential(*modules_pool)
    ...
    def forward(self, x):
        ...
        if self.modified:
            ...
            x_even_3 = x_odd.mul(torch.exp(self.phi_3(x_even)))
            x_odd_3 = x_even.mul(torch.exp(self.psi_3(x_odd)))

            x_even_5 = x_odd.mul(torch.exp(self.phi(x_even)))
            x_odd_5 = x_even.mul(torch.exp(self.psi(x_odd)))

            x_even_1 = x_odd.mul(torch.exp(self.phi_1(x_even)))
            x_odd_1 = x_even.mul(torch.exp(self.psi_1(x_odd)))

            x_even_c = torch.cat((x_even_3, x_even_5, x_even_1), dim=1)
            x_odd_c = torch.cat((x_odd_3, x_odd_5, x_odd_1), dim=1)

            d = self.pool(x_even_c)
            c = self.pool(x_odd_c)
